...
func InitGithubDownloader() {
	...
	EsharqFile := EsharqDirectory

	stat, err := os.Stat(EsharqFile)
	if err != nil {
		return
	}
	...
	// dev
	if stat.IsDir() {
		EsharqFile = path.Join(EsharqFile, "main.js")
	}

	// Check hash of installed version if exists
	b, err := os.ReadFile(EsharqFile)
	if err != nil {
		return
	}

	Log.Debug("Found existing Esharq Install. Checking for hash...")

	re := regexp.MustCompile(`// Esharq (\w+)`)
...
func installLatestBuilds() (retErr error) {
	...
	out, err := os.OpenFile(EsharqDirectory, os.O_WRONLY|os.O_CREATE|os.O_TRUNC, 0644)
	if err != nil {
		Log.Error("Failed to create", EsharqDirectory+":", err)
		retErr = err
		return
	}
	read, err := io.Copy(out, res.Body)
	if err != nil {
		Log.Error("Failed to download to", EsharqDirectory+":", err)
		retErr = err
		return
	}
	...
	_ = FixOwnership(EsharqDirectory)
