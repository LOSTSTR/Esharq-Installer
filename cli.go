...
func main() {
	...
	var installFlag = flag.Bool("install", false, "Install Esharq")
	var updateFlag = flag.Bool("repair", false, "Repair Esharq")
	var uninstallFlag = flag.Bool("uninstall", false, "Uninstall Esharq")
	...
	if *versionFlag {
		fmt.Println("EsharqInstaller Cli", buildinfo.InstallerTag, "("+buildinfo.InstallerGitHash+")")
		fmt.Println("Copyright (C) 2026 Vendicated, thororen1234, Vencord, and Esharq contributors")
	...
	if !SliceContainsFunc(switches, func(b *bool) bool { return *b }) {
		...
		go func() {
			...
			if IsSelfOutdated {
				...
				Log.Warn("To update, select the 'Update EsharqInstaller' option to update, or run with --update-self")
			}
		}()
	...
	if install {
		...
		Log.Info("Downloading latest Esharq files...")
...
func HandleScuffedInstall() {
	...
	fmt.Println("Otherwise, Esharq will likely not work.")
}
