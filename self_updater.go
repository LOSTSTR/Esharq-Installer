...
func GetInstallerDownloadLink() string {
	const BaseUrl = "https://github.com/LOSTSTR/Esharq-Installer/releases/latest/download/"
	switch runtime.GOOS {
	case "windows":
		filename := Ternary(buildinfo.UiType == buildinfo.UiTypeCli, "EsharqInstallerCli.exe", "EsharqInstaller.exe")
		return BaseUrl + filename
	case "darwin":
		switch runtime.GOARCH {
		case "amd64":
			return BaseUrl + "EsharqInstaller-darwin-x64.zip"
		case "arm64":
			return BaseUrl + "EsharqInstaller-darwin-arm64.zip"
		default:
			return ""
		}
	case "linux":
		return BaseUrl + "EsharqInstallerCli-linux"
	default:
		return ""
	}
}
...
func UpdateSelf() error {
	...
	tmp, err := os.CreateTemp(ownExeDir, "EsharqInstallerUpdate")
	if err != nil {
