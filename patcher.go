...
func init() {
	if dir := os.Getenv("EQUICORD_USER_DATA_DIR"); dir != "" {
		...
		Log.Debug("Using DISCORD_USER_DATA_DIR/../EsharqData")
		BaseDir = path.Join(dir, "..", "EsharqData")
	} else {
		Log.Debug("Using UserConfig")
		BaseDir = appdir.New("Esharq").UserConfig()
	}
	...
	if dir != "" {
		...
		EsharqDirectory = path.Join(BaseDir, "esharq.asar")
	}
}
...
func patchAppAsar(dir string, isSystemElectron bool) (err error) {
	...
	if err := WriteAppAsar(appAsar, EsharqDirectory); err != nil {
		return err
	}
...
func (di *DiscordInstall) patch() error {
	...
	if di.isFlatpak {
		...
		Log.Debug("This is a flatpak. Trying to grant the Flatpak access to", EsharqDirectory+"...")
		...
		args = append(args, "override", name, "--filesystem="+EsharqDirectory)
		...
		if err != nil {
			return errors.New("Failed to grant Discord Flatpak access to " + EsharqDirectory + ": " + err.Error())
		}
	}
	return nil
}
...
func isEsharqLoaderAppAsar(appAsar string) (bool, error) {
...
func cleanupDesyncedPatchedInstall(dir string, isSystemElectron bool) (bool, error) {
	...
	isLoader, err := isEsharqLoaderAppAsar(appAsar)
	if err != nil {
		return false, err
	}
	if isLoader {
	...
	Log.Warn("Detected a patched install with a non-Esharq app.asar. Discord was likely updated while patched; removing stale _app.asar")
